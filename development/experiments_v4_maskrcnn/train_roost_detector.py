...
import adaptors_fpn
from adaptors_fpn import CustomResize, transform_instance_annotations
...
parser.add_argument('--adaptor', default='None', type=str, help='linear or multi-layer adaptor')
parser.add_argument('--channel_strategy', required=True, type=int, help='channel strategy')
...
ARRAY_DIR = "/work/pi_drsheldon_umass_edu/roosts/RadarNPZ" # "/gypsum/datasets/RadarNPZ"

if args.channel_strategy == 1:
    CHANNELS = [("reflectivity", 0.5)]
elif args.channel_strategy == 2:
    CHANNELS = [("reflectivity", 0.5), ("velocity", 0.5)]
elif args.channel_strategy == 3:
    CHANNELS = [("reflectivity", 0.5), ("velocity", 0.5), ("spectrum_width", 0.5)]
elif args.channel_strategy == 4:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5)]
elif args.channel_strategy == 5:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5), ("velocity", 0.5)]
elif args.channel_strategy == 6:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5), ("velocity", 0.5), ("spectrum_width", 0.5)]
elif args.channel_strategy == 7:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5),
                ("velocity", 0.5), ('velocity', 1.5),
                ("spectrum_width", 0.5), ("spectrum_width", 1.5)]
elif args.channel_strategy == 8:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5), ("reflectivity", 2.5),
                ("velocity", 0.5), ('velocity', 1.5), ("velocity", 2.5),
                ("spectrum_width", 0.5), ("spectrum_width", 1.5), ("spectrum_width", 2.5)]
elif args.channel_strategy == 9:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5), ("reflectivity", 2.5), ("reflectivity", 3.5),
                ("velocity", 0.5), ('velocity', 1.5), ("velocity", 2.5), ("velocity", 3.5),
                ("spectrum_width", 0.5), ("spectrum_width", 1.5), ("spectrum_width", 2.5), ("spectrum_width", 3.5)]
elif args.channel_strategy == 10:
    CHANNELS = [("reflectivity", 0.5), ("reflectivity", 1.5), ("reflectivity", 2.5),
                ("reflectivity", 3.5), ("reflectivity", 4.5),
                ("velocity", 0.5), ('velocity', 1.5), ("velocity", 2.5), ('velocity', 3.5), ("velocity", 4.5),
                ("spectrum_width", 0.5), ("spectrum_width", 1.5), ("spectrum_width", 2.5),
                ("spectrum_width", 3.5), ("spectrum_width", 4.5)]
else:
    sys.exit(f'Unknown channel strategy. Program exited.')
...
if 'resnet50' in args.network or 'r50' in args.network:
    imgnet_pretrained_url = "https://dl.fbaipublicfiles.com/detectron2/ImageNetPretrained/MSRA/R-50.pkl"
elif 'resnet101' in args.network or 'r101' in args.network:
...
if args.adaptor != 'None':
    assert args.adaptor in ['linear', 'multi-layer'], "Unknown adaptor type"
    cfg.MODEL.BACKBONE.NAME = 'build_adaptor_resnet_fpn_backbone'
    cfg.ADAPTOR_TYPE = args.adaptor
    cfg.ADAPTOR_IN_CHANNELS = len(CHANNELS) # assume single frame
...
def mapper(dataset_dict):
    ...
    arrays = np.load(dataset_dict["file_name"])

    array = arrays["array"]
    image = np.stack([
        NORMALIZERS[attr](array[attributes.index(attr), elevations.index(elev), :, :]) for (attr, elev) in CHANNELS
    ], axis=-1)
    np.nan_to_num(image, copy=False, nan=0.0)
    image = (image * 255).astype(np.uint8)
    ...
    if args.segmentation:
        dualpol = arrays["dualpol_array"][1, 0, :, :] # correlation
        dualpol[np.isnan(dualpol)] = DUALPOL_THRESHOLD + 0.1
        mask = np.ones(dualpol.shape)
        mask[dualpol > DUALPOL_THRESHOLD] = 0 # rain or NAN
    ...
    for annotation in dataset_dict.pop("annotations"):
        if args.segmentation:
            ...
            instance_mask[bbox[1]:bbox[3], bbox[0]:bbox[2]] = mask[bbox[1]:bbox[3], bbox[0]:bbox[2]]
            # annotation["segmentation"] = pycocotools.mask.encode(np.asarray(
            #     instance_mask.astype(np.uint8), order="F"
            # ))
            annotation["segmentation"] = instance_mask.astype(np.uint8)
        anno = transform_instance_annotations(annotation, transforms, image.shape[1:])
        annos.append(anno)
    instances = detection_utils.annotations_to_instances(annos, image.shape[1:], mask_format="bitmask")
